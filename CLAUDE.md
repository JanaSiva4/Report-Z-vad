...
The platform team provides three values when provisioning the GCP project:

| Input | Purpose |
|---|---|
| `GCP_PROJECT_ID` | GCP project ID — goes into `container/config.env` |
| `GCS_BUCKET` | Cloud Storage bucket name — goes into `container/config.env` |
| `SA_NAME` | Service account email — used locally for `gcloud` impersonation, never in the container |
